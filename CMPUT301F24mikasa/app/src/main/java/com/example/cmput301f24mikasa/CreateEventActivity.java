...
import com.example.cmput301f24mikasa.EventPosterActivity;
import com.google.android.gms.tasks.OnFailureListener;
import com.google.android.gms.tasks.OnSuccessListener;
import com.google.firebase.firestore.CollectionReference;
import com.google.firebase.firestore.DocumentReference;
...
public class CreateEventActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // Disable the QR code and poster buttons initially
        ...
        btnCreateEvent.setOnClickListener(v -> {
            ...
            if (imageUri != null) {
                createEvent(title, date, price, desc);
                btnGenerateQRCode.setEnabled(true); // Enable QR code generation after event creation
            } else {
                Toast.makeText(CreateEventActivity.this, "Please upload an image", Toast.LENGTH_SHORT).show();
            }
        });
        ...
        btnGenerateQRCode.setOnClickListener(v -> {
            if (eventID != null) {  // Ensure eventID is set before generating QR code
                ...
                eventRef.document(eventID).update("qrRef", eventID).addOnSuccessListener(aVoid -> {
                    Toast.makeText(CreateEventActivity.this, "QR Code successfully generated", Toast.LENGTH_SHORT).show();
                    btnCreatePoster.setEnabled(true); // Enable poster creation after QR code generation
                }).addOnFailureListener(e ->
        ...
        btnCreatePoster.setOnClickListener(v -> {
            if (imageUri != null && eventID != null) {
                ...
                Toast.makeText(CreateEventActivity.this, "Ensure event and QR code are created before proceeding.", Toast.LENGTH_SHORT).show();
            }
        });
    }
